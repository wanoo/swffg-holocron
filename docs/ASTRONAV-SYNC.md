...
> **`swffg-astronav`** (`~/Documents/Dev/star-wars JDR/swffg-astronav`, publié sur
> https://github.com/wanoo/swffg-astronav — actuellement **v1.5.1**).
> Décisions verrouillées le 2026-07-12. But : que les deux projets avancent alignés.

## 1. Architecture décidée

- **`swffg-astronav`** = LA brique **astrogation réutilisable** (module Foundry autonome,
...
- **`swffg-holocron`** = le **poste de commande de campagne** (ce projet). Il a une
  **dépendance OBLIGATOIRE** à `swffg-astronav` (à déclarer `relationships.requires` le jour
...
| Overlay hyperroutes | **Ajouté au module** (v1.5.1) : bascules Grandes/Mineures. |
| Recherche | Via le système / datalist (delta accepté). |

## 3. Contrat d'API du module `swffg-astronav` (ce que Holocron consomme)
...
- `modules/swffg-astronav/data/planets.json` — ~6800 systèmes. `img` = **chemins locaux**
  `modules/swffg-astronav/img/planets/…` (plus de hotlink Wookieepedia).
- `modules/swffg-astronav/data/lanes.json` — 63 hyperroutes (`planets[]` ordonnés + `pts[]`).
- `modules/swffg-astronav/img/galaxy-map.jpg` — fond GFFA 5400². Calibration :
  `CAL = {cx:2699.5, cy:2490, k:2.155, size:5400}`, `posOf(p)=[cx+xy0*k, cy-xy1*k]`.
- Packs : `swffg-astronav.planetes` (JournalEntry MEJ *Place*) + `swffg-astronav.macros` (Macro).
...
1. Quand le module Foundry Holocron sera créé : `relationships.requires` = `swffg-astronav`
...
5. Ne PAS ré-implémenter la carte / le calcul : tout vient de `swffg-astronav`.

## 6. Références

- Module : https://github.com/wanoo/swffg-astronav (v1.5.1) — install : manifeste `releases/latest`.
- Source module : `~/Documents/Dev/star-wars JDR/swffg-astronav` (repo git indépendant).
