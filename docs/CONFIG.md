...
```jsonc
{
  ...
  "journals": {
    ...
    "mjSheets": "🔥 Fronts & secrets (MJ)",  // DOSSIER d'accueil des fiches MJ (Front/Secret/Prépa)
    "shipNotes": "7y1NXkJxRJVlXwH8:pPQ2rX0hAZfvG0QP"
  },

  // Registre des personnages (mentions cliquables dans les textes MJ + backrefs
  // « Mentionné dans »). GÉNÉRÉ automatiquement — POST /api/gm/registry ou
  // installation du module ; les entrées écrites à la main sont conservées et
  // seulement complétées. MJ ONLY (absent de /api/content/config pour un joueur).
  "registry": [
    { "kind": "pc", "id": "<idActor>", "forms": ["Nom", "Surnom", "Faute d'orthographe"] },
    { "kind": "npc", "id": "<idActeurPNJ ou idFicheCC>", "forms": ["Moff Kalast", "Kalast"], "auto": true }
  ],
...
| `flags.holocron.dossiers` | journal 🗂️ | dossiers MJ narratifs — `{entityId: {name, role, statut, veut, levier, indices, attitude, replique, advId}}` ; affichés sur les fiches (résolution par id puis par nom), servis par `GET /api/gm/dossiers`. **Édition** : panneau « ✎ Modifier » de la fiche → `PUT /api/gm/dossiers/<entityId>` (patch PARTIEL : seuls les champs envoyés sont touchés, un champ vide est retiré) ; amorce `POST /api/gm/dossiers/seed` (`{dryRun}`) qui pré-remplit `role`/`statut` depuis `flags.holocron.statut` des fiches CC, strictement additive. Les backrefs « Mentionné dans » sont CALCULÉES depuis les chapitres + `registry` (`GET /api/gm/backrefs`) |
| `flags.holocron.mjSheet` | journal fiche MJ | marqueur d'idempotence des **gabarits MJ** (`front` \| `secret` \| `prepa`) — voir « Fiches MJ » ci-dessous. Ce sont les TAGS qui font foi, jamais ce flag |
...
## Fiches MJ : Front / Secret / Prépa (gabarits Campaign Codex privés)

Trois gabarits de préparation vivent comme des **fiches Campaign Codex `tag`**
(la fiche GÉNÉRIQUE de CC) et non comme des flags maison : le MJ les organise,
les lie et les tague **depuis Foundry** exactement comme le reste de sa campagne.

| Gabarit | Libellé sheet | Tag | Champs (`flags.campaign-codex.data.*`) | États |
|---|---|---|---|---|
| Front / Menace | `Front` | `mj:front` | `intention`, `horloge`, `cible`, `signes` | `mj:front-actif` (défaut) · `mj:front-eteint` |
| Secret / Vérité | `Secret` | `mj:secret` | `verite`, `indices`, `revelableA` | `mj:secret-seme` (absent = pas encore semé) |
| Préparation de séance | `Prépa` | `mj:prepa` | `semer`, `questions`, `checklist` | — |

Forme exacte de la fiche (`server/lib/transform/mj-sheets.mjs`) :
`flags.campaign-codex.type = "tag"`, `data.tagMode = true`,
`data.sheetTypeLabelOverride` = le libellé, `flags.core.sheetClass =
"campaign-codex.TagSheet"`, `ownership.default = 0` (**MJ only strict**).
Les champs sont **miroités en HTML lisible** dans `data.description` — c'est ce
que la sheet CC affiche dans Foundry. Les tags sont écrits **des deux côtés**
(`data.tags` ET `flags.asset-librarian.filterTag`) et relus des deux côtés : un
tag posé à la main dans Asset Librarian est reconnu, jamais écrasé. Les liens
utilisent les relations **natives** CC (`associates`, `linkedLocations`,
`linkedActor`, `linkedStandardJournals`) — un front pointe ses PNJ, un secret
son lieu.

Ces fiches sont des **nœuds de la carte de campagne** (`buildCatalog` →
`type: "mj-front" | "mj-secret" | "mj-prepa"`, `mjKind`, `mjState`), avec leurs
relations CC en arêtes. Une fiche `tag` sans tag `mj:*` (une note de joueur
promue, par exemple) reste hors carte.

Routes : `GET /api/gm/mj-sheets[?kind=]` (+ les gabarits), `GET /api/gm/mj-sheets/<id>`,
`POST /api/gm/mj-sheets` (création), `PUT /api/gm/mj-sheets/<id>` (patch partiel),
`POST /api/gm/mj-sheets/migrate-fronts` (l'ancienne liste plate `gm:cfg:fronts`
→ fiches CC, non destructive et idempotente : la config n'est pas vidée et un
front déjà représenté n'est pas recréé). Dossier d'accueil :
`config.journals.mjSheets` (défaut `🔥 Fronts & secrets (MJ)`) — déplaçable
librement dans Foundry, le service suit.

## Préparer une séance (routes dérivées)

| Route | Rôle |
|---|---|
| `GET /api/gm/act-check/<actId>` | **Checklist prêt-à-jouer DÉRIVÉE** (remplace la liste manuelle de `gm:cfg:session`) : `[{severity: error\|warn\|info, code, beatId?, ref?, message, fix}]` — beat sans lieu ni PNJ, beat ⚔️ sans rencontre ou pointant une rencontre disparue, séquence supprimée, référence morte (entité effacée de Foundry), acte sans handout, secret `mj:secret` semé nulle part. Trié par gravité, chaque ligne porte son geste réparateur |
| `POST /api/gm/encounters/import-scan` | **Aperçu SEUL** des combats en texte de la bible : parcourt les chapitres, repère les blocs `<pre class="combat">` / ```` ```combat ````, rend `{chapters, found: [{chapterId, chapterName, exists, reason?, encounter}], news}`. **Aucune écriture** |
| `POST /api/gm/encounters/import` | Import de la **seule sélection** cochée (`{ids: [...]}`) vers `flags.holocron.encounters`. Ids stables (dérivés du contenu) : ré-importer met à jour, ne duplique jamais |
| `POST /api/gm/scenario/parse` | Saisie rapide : `{text}` → `{beats: [{kind, title, note}]}` (titres markdown, lignes en MAJUSCULES, titres soulignés, « Scène 3 : … », paragraphes autonomes ; type deviné 🎭⚔️🗒️🖼️). Aucune écriture — le MJ retouche puis enregistre le storyboard |
| `GET` / `POST /api/gm/registry` | Registre des personnages (`config.registry`). Le POST le **régénère** depuis les acteurs PJ, les acteurs du dossier PNJ du monde et les fiches CC `npc` : fusion NON destructive (les formes écrites à la main sont gardées puis complétées, les entrées orphelines conservées), entrées générées marquées `auto: true`. `{dryRun: true}` pour l'aperçu. Même moulinette à l'installation du module (`ensureRegistry`) |

⚠️ `config.registry` n'est **pas** servi dans `/api/content/config` pour un
joueur : régénéré automatiquement, il listerait le nom et l'id de tous les PNJ
de la campagne. Il n'est lu que par des vues MJ.
