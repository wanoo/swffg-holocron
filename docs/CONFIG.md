...
## Monk's Enhanced Journal (types de fiches)

Si le module **Monk's Enhanced Journal** est installé, ses fiches typées
(personne, lieu, organisation, boutique…) sont reconnues automatiquement — aucune
config à ajouter. Le méta structuré de la page (`flags.monks-enhanced-journal` :
`type`, `role`, `location`, `attributes`, `relationships`) est extrait et affiché
en **carte d'identité** au-dessus du texte : type, attributs (espèce, faction, vie…),
rattachement et **relations cliquables** vers les autres journaux visibles. Le
`role` (Enemy/Ally/…) et l'attribut `life` (« Mort ») alimentent en repli la pastille
de statut et le marqueur † (les flags `holocron.statut/mort` restent prioritaires).
Les notes MJ privées de la page (clé par-utilisateur) ne sont jamais exposées, et
les relations `hidden` ne sortent que pour le MJ.

> **Fraîcheur** : le `_stats.modifiedTime` d'une *page* ne remonte pas sur le
> journal parent, et l'index léger n'a pas les pages ; les journaux MEJ sont donc
> re-synchronisés à chaque tick (ensemble borné). Édition MEJ dans Foundry →
> visible ≤ un cycle `SYNC_INTERVAL_S` (ou bouton ⟳ MJ).
