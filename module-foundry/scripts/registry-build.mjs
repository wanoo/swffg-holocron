// registry-build.mjs — GÉNÉRATION DU REGISTRE DES PERSONNAGES (côté module).
//
...
// PURE et sans I/O.
//
// ⚠️ COPIE CONFORME de `server/lib/transform/registry.mjs`. La duplication est
// VOULUE et surveillée : l'image Docker du service ne contient que `server/` et
// `public/` (le module Foundry n'y est pas), et le module Foundry ne peut pas
// importer le serveur. `server/test/registry.test.mjs` compare les deux
// implémentations sur un même jeu de noms — elles ne peuvent pas diverger sans
// que les tests tombent. Toute modification doit être reportée des DEUX côtés.
